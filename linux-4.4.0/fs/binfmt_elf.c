...
DECLARE_PER_CPU(unsigned long, pt_buffer_cpu);
...
//testing by JX
	if(fill_pt_note(&info->pt) == 0){
		info->size += notesize(&info->pt);
	}
//end testing by JX
			   ...
			   struct coredump_params *cprm)
{
	...
	do {
		...
		//added by JX
		if(first && info->pt.data &&
				!writenote(&info->pt, cprm))
			return 0;
		//end adding by JX
