...
static struct tracepoint *exec_pt; 
static struct tracepoint *switch_pt; 
static struct tracepoint *fork_pt;
...
//please make sure no operations inside this function will do something expensive;
//please make sure no operations inside this function will cause data race;
...
static int simple_pt_buffer_init(int cpu){
	...
	if (!pt_buffer) {
		...
		if (!pt_buffer) {
			printk("Cannot allocate sufficient memory for PT buffer\n");
			return -ENOMEM;
		}
...
static void pt_cpu_init(void* arg){
	...
	if (val & TRACE_EN){
		printk("The PT already started\n");
		if(wrmsrl_safe(MSR_IA32_PT_CTL, val & ~TRACE_EN)<0){
			pt_error = -1;
			return ;
		}
	}
	...
	//prepare the logging buffer and offset for PT to write
...
static int cpuid_test(void){
	...
	cpuid(0, &a, &b, &c, &d);
	if(a < 0x14){
		printk("The CPU ID is not sufficient to support PT\n");
		return -1;
	}
	cpuid_count(0x07, 0, &a, &b, &c, &d);
	if(!(b & BIT(25))){
		printk("No PT support\n");
		return -1;
	}
	...
	if(!!(b&BIT(0)))
		printk("Has cr3 match support\n");		
	return 0;
}

//first test: using a single_range ring buffer. 
static int check_intel_pt(void (*probe)(void *, struct task_struct*, pid_t, struct linux_binprm*)){
	...
	on_each_cpu(pt_cpu_init, NULL, 1);

	if(pt_error){
		printk("PT initialization does not complete\n");
		return -1;		
	}	

	//trace execution of new process
	exec_pt = (struct tracepoint*) kallsyms_lookup_name("__tracepoint_sched_process_exec");

	if(!exec_pt){
		printk("Warning: Cannot register exit traceevent\n");
		return -EIO;	
	}
	...
	if(!fork_pt){
		printk("Warning: Cannot register fork traceevent\n");
		return -EIO;
	}
	...
	exit_pt =  (struct tracepoint*) kallsyms_lookup_name("__tracepoint_sched_process_exit");
...
static void stop_pt(void * arg){
	...
	rdmsrl_safe(MSR_IA32_PT_CTL, &ctl);
	rdmsrl_safe(MSR_IA32_PT_STATUS, &status);
	
	if(!(ctl & TRACE_EN))
		printk("Intel PT is not enabled\n");
	
	if(status & PT_ERROR){
		printk("Intel PT is in error status");
		wrmsrl_safe(MSR_IA32_PT_STATUS, 0);
	}	
	wrmsrl_safe(MSR_IA32_PT_CTL, 0LL);
}
...
static int release_intel_pt(void (*probe)(void *, struct task_struct*, pid_t, struct linux_binprm*)){
	...
	printk("Release the PT tracing process\n");
	...
	if(exec_pt)
		tracepoint_probe_unregister(exec_pt, (void*)probe, NULL);

	if(fork_pt)
		tracepoint_probe_unregister(fork_pt, (void*)probe_sched_fork, NULL);

	if(switch_pt)
		tracepoint_probe_unregister(switch_pt, (void*)probe_sched_switch, NULL);

	if(exit_pt)
		tracepoint_probe_unregister(exit_pt, (void*)probe_sched_exit, NULL);
...
//added by JX
	if(!retval && new_rlim && resource == RLIMIT_PTBUF)
	{
		...
		if(!no_lim && new_rlim->rlim_cur == 0)
			release_intel_pt(probe_sched_process_exec);
	}
//end adding by JX
	return retval;
}
